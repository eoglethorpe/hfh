...
function sortParks(a, b) {
  var _a = a.feature.properties.general_info_registration_number;
  var _b = b.feature.properties.general_info_registration_number;
  if (_a < _b) {
    return -1;
  }
  if (_a > _b) {
...
L.Control.Search = L.Control.extend({
    ...
    if (e.keyCode === 38 || e.keyCode === 40) {
      ...
      if (this.input.value.length > 2) {
        ...
        var results = _.take(_.filter(this.options.data, function(x) {
          return x.feature.properties.general_info_registration_number.toUpperCase().indexOf(value.toUpperCase()) > -1;
        }).sort(sortParks), 10);
        _.map(results, function(x) {
          var a = L.DomUtil.create('a', 'list-group-item');
          a.href = '';
          a.setAttribute('data-result-name', x.feature.properties.general_info_registration_number);
          a.innerHTML = x.feature.properties.general_info_registration_number;
  ...
  itemSelected: function(e) {
    ...
    var feature = _.find(this.options.data, function(x) {
      return x.feature.properties.general_info_registration_number === this.input.value;
    }, this);
    if (feature) {
		//resetStyle() can be used here
		 feature.setStyle({
				fillColor: '#c0392b',
				color: '#c0392b'
			});
